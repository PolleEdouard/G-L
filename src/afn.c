...
// Chaque état de l'AFD est un masque de bits (int) représentant un
// sous-ensemble d'états AFN
int dfa_states[MAX_DFA_STATES]; // Stocke le masque binaire de chaque état AFD
int dfa_trans[MAX_DFA_STATES]
             [MAX_ALPHABET_SIZE]; // Table de transition de l'AFD
...
void enqueue(int mask) { queue[rear++] = mask; }
...
// Trouve l'index d'un état AFD existant à partir de son masque. Retourne -1 si
// non trouvé.
...
// Génère une chaîne représentant le sous-ensemble d'états AFN pour un masque
// donné
void get_subset_label(int mask, int states, char *buffer) {
  char *ptr = buffer;
  ptr += sprintf(ptr, "{ ");
  bool first = true;
  for (int i = 0; i < states; i++) {
...
// Exporte l'AFD au format Graphviz DOT
void export_dot(const char *filename) {
  ...
  // Flèche invisible pour l'état initial
  ...
  char label[256];
  for (int i = 0; i < num_dfa_states; i++) {
    get_subset_label(dfa_states[i], num_nfa_states, label);
    const char *shape = dfa_final[i] ? "doublecircle" : "circle";
    fprintf(f, "    %d [label=\"%s\", shape=%s];\n", i, label, shape);
  }
...
int main() {
  ...
  if (strlen(alphabet) != (size_t)alphabet_size) {
    printf(
        "Le nombre de symboles ne correspond pas a la taille de l'alphabet.\n");
    return 1;
  }
  ...
  for (int i = 0; i < num_nfa_states; i++) {
    ...
    for (int s = 0; s < alphabet_size; s++) {
      printf("  Transitions pour '%c' (%d chiffres): ", alphabet[s],
             num_nfa_states);
  ...
  // Générer le fichier DOT et l'image PNG dans le dossier results/
  export_dot("results/afd.dot");

  printf("Génération de l'image PNG...\n");
  int ret = system("dot -Tpng results/afd.dot -o results/afd.png");
  if (ret == 0) {
    printf("Image générée avec succès: results/afd.png\n");
  } else {
    printf("Erreur lors de la génération de l'image (vérifiez que Graphviz est "
           "installé).\n");
  }
